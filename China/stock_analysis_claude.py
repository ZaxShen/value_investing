...
# TODO: Pay attention to the timeout of the API calls
# Get the list of industry names
stock_board_industry_name_em_arr = ak.stock_board_industry_name_em()["板块名称"]
# Global variable to hold the list of industry names
# Extract all stocks from the industry board
industry_stock_mapping_df = pd.DataFrame(columns=["行业", "代码"])
for industry_name in stock_board_industry_name_em_arr:
    stock_board_industry_cons_em_df = ak.stock_board_industry_cons_em(
        symbol=industry_name
    )
    for stock_code in stock_board_industry_cons_em_df["代码"]:
        industry_stock_mapping_df.loc[len(industry_stock_mapping_df)] = [
            industry_name,
            stock_code,
        ]
# Release the memory of stock_board_industry_name_em_arr
del stock_board_industry_name_em_arr
...
async def process_single_stock(stock_code, stock_name, industry_name, days=29):
    ...
    async with REQUEST_SEMAPHORE:  # Limit concurrent requests
        ...
        try:
            ...
            # Calculate avg change percentage
            stock_price_avg_change_percentage = stock_individual_fund_flow_df[
                "涨跌幅"
            ].mean()
            stock_price_avg_change_percentage = round(
                stock_price_avg_change_percentage, 2
            )
            ...
            return [
                ...
                stock_main_net_flow,
                stock_price_change_percentage,
                stock_price_avg_change_percentage,
                stock_60d_change,
                stock_ytd_change,
            ]
...
async def main():
    ...
    DIR_PATH = "data/stocks/reports"

    # Output the stock_filter_df to a CSV file
    stock_filter_df.to_csv(
        f"{DIR_PATH}/股票筛选报告-raw-{last_date_str}.csv", index=True
    )
    print(f"Report saved to {DIR_PATH}/股票筛选报告-{last_date_str}.csv")
    ...
    df.to_csv(f"{DIR_PATH}/股票筛选报告-{last_date_str}.csv", index=True)
    print(f"Filtered report saved to {DIR_PATH}/股票筛选报告-{last_date_str}.csv")
