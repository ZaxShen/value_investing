...
class FhpsFilterConfig(BaseModel):
    ...
    max_price_change_percent: float = 10.0  # Maximum price change percentage for filtering
...
def load_config(
    ...
    if "akshare" in config_data:
        ...
        akshare_config = StockIndividualFundFlowConfig(**configs.akshare.get("stock_individual_fund_flow", {}))
...
class FhpsFilter:
    ...
    def __init__(
        ...
        # Load configuration
        self.akshare_config, self.filter_config, self.file_config = load_config(config_name)
    ...
    async def get_stock_price_async(self, stock_code: str, date: datetime) -> Optional[float]:
        """
    ...
    def get_stock_industry(self, stock_code: str) -> str:
        ...
        try:
            ...
            return industry_row.iloc[0].get("板块名称", "未知")
    ...
    async def run_analysis(
        ...
        try:
            ...
            # Fetch FHPS data
            self.logger.info(f"Fetching FHPS data for date: {self.FHPS_DATE}")
            try:
                ...
                error_msg = f"Failed to fetch FHPS data for date {self.FHPS_DATE}: {str(api_error)}"
            ...
            self.logger.info(f"Initial FHPS data contains {len(stock_fhps_em_df)} records")
            ...
            self.logger.info(f"After ex-dividend date filter (< today): {len(df_filtered)} stocks")
            
            # Apply minimum transfer ratio filter
            df_filtered = df_filtered[df_filtered["送转股份-送转总比例"] >= self.MIN_TRANSFER_RATIO]
            self.logger.info(f"After transfer ratio filter (>= {self.MIN_TRANSFER_RATIO}): {len(df_filtered)} stocks")
            ...
            self.logger.info(f"Getting prices for {len(df_filtered)} FHPS stocks in {len(price_batches)} batches")
            ...
            for batch_idx, batch in enumerate(price_batches):
                if _progress and _parent_task_id:
                    ...
                    _progress.update(
                        ...
                        description=f"📈 Getting prices {batch_idx + 1}/{len(price_batches)}..."
                    )
                ...
                for original_idx, row in batch:
                    ...
                    try:
                        # Get prices for price change calculation
                        price_results_batch = await asyncio.gather(
                            self.get_stock_price_async(stock_code, ex_date),
                            self.get_stock_price_async(stock_code, today),
                            return_exceptions=True
                        )
                        
                        ex_price = price_results_batch[0] if not isinstance(price_results_batch[0], Exception) else None
                        today_price = price_results_batch[1] if not isinstance(price_results_batch[1], Exception) else None
            ...
            filtered_stocks = [
                stock for stock in price_results 
                if stock['price_change_pct'] < self.MAX_PRICE_CHANGE_PERCENT
            ]
            
            self.logger.info(f"After price change filter (<{self.MAX_PRICE_CHANGE_PERCENT}%): {len(filtered_stocks)} stocks")
            ...
            if _progress and _parent_task_id:
                _progress.update(
                    _parent_task_id,
                    completed=75,
                    description=f"📊 Enriching {len(filtered_stocks)} filtered stocks..."
                )
            
            all_results = []
            
            for i, stock_info in enumerate(filtered_stocks):
                if _progress and _parent_task_id and i % 5 == 0:  # Update progress every 5 stocks
                    progress_pct = 75 + (i / len(filtered_stocks)) * 15
                    _progress.update(
                        _parent_task_id,
                        completed=progress_pct,
                        description=f"📊 Enriching stock {i + 1}/{len(filtered_stocks)}..."
                    )
                ...
                try:
                    ...
                    result = {
                        ...
                        **market_data,
                        ...
                        "除权除息日": stock_info['row']["除权除息日"].strftime("%Y-%m-%d") if isinstance(stock_info['row']["除权除息日"], datetime) else str(stock_info['row']["除权除息日"]),
                        "除权除息日股价": stock_info['ex_price'],
                        f"{today.strftime('%Y%m%d')}股价": stock_info['today_price'],
                        "自除权出息日起涨跌幅%": stock_info['price_change_pct'],
                        # Fund flow and price change data
                        **fund_flow_data,
                    }
            ...
            if all_results:
                ...
                result_df = result_df.sort_values(by=["自除权出息日起涨跌幅%"], ascending=True)
                ...
                self.logger.info(f"FHPS analysis completed. Report saved to: {output_path}")
