...
class FileConfig(BaseModel):
    ...
    config_name: str = "FHPS_CACHING_PROD"
    description: str = ""
    version: str = "1.9.3"
    ...
    async def phase1_caching(self) -> None:
        ...
        for year in self.config.phase1_caching.historical_years:
            ...
            if self.config.phase1_caching.skip_existing and os.path.exists(cache_path):
                self.logger.info(f"✅ CACHED FILE EXISTS - Skipping {year}: {cache_path}")
            ...
            try:
                ...
                df_filtered = stock_fhps_em_df.dropna(subset=["送转股份-送转总比例"]).copy()
                
                # Ensure stock codes are strings with proper 6-digit format
                if "代码" in df_filtered.columns:
                    df_filtered.loc[:, "代码"] = df_filtered["代码"].apply(lambda x: str(x).zfill(6))
                
                # Reset index and save to cache
                df_filtered.reset_index(drop=True).to_csv(cache_path, index=False, encoding="utf-8-sig")
                
                self.logger.info(f"✅ Cached {len(df_filtered)} FHPS records for {year}: {cache_path}")
                print(f"✅ Cached {len(df_filtered)} FHPS records for {year}: {cache_path}")
    ...
    async def phase2_caching(self) -> None:
        ...
        for year in self.config.phase2_caching.target_years:
            ...
            try:
                ...
                self.logger.info(f"Found {len(df_past_dates)} stocks with past ex-dividend dates for {year}")
                print(f"📊 Found {len(df_past_dates)} stocks with past ex-dividend dates for {year}")
                ...
                for _, row in df_past_dates.iterrows():
                    ...
                    try:
                        ...
                        self.logger.warning(f"Failed to get price for {stock_code} on {ex_date}: {e}")
                ...
                # Reset index and save to cache
                df_enriched.reset_index(drop=True).to_csv(cache_path, index=False, encoding="utf-8-sig")
                
                self.logger.info(f"✅ Cached {len(df_enriched)} enriched FHPS records for {year}: {cache_path}")
                print(f"✅ Cached {len(df_enriched)} enriched FHPS records for {year}: {cache_path}")
                ...
                df_enriched.reset_index(drop=True).to_csv(latest_path, index=False, encoding="utf-8-sig")
                
                self.logger.info(f"✅ Created latest file: {latest_path}")
                print(f"✅ Created latest file: {latest_path}")
                
            except Exception as e:
                error_msg = f"Failed to cache FHPS data with prices for year {year}: {str(e)}"
                self.logger.error(error_msg)
                print(f"❌ {error_msg}")
                raise RuntimeError(error_msg)
