...
class TestAsyncioTimeoutIntegration:
    ...
    async def test_stock_filter_timeout_wrapper(self):
        ...
        with patch.object(stock_filter, '_fetch_stock_fund_flow_sync', side_effect=slow_api_call):
            start_time = asyncio.get_event_loop().time()
            
            # This should timeout and return None (handled gracefully)
            result = await stock_filter.process_single_stock_async("000001", "平安银行", 29)
    ...
    async def test_concurrent_timeouts(self):
        ...
        def variable_timeout_api(*args, **kwargs):
            ...
            if call_count == 1:
                ...
                return pd.DataFrame({"日期": ["2025-01-01"], "主力净流入-净额": [1000000]})  # Second succeeds
            else:
        ...
        with patch.object(stock_filter, '_fetch_stock_fund_flow_sync', 
            ...
            assert elapsed < 2.0, f"Concurrent timeouts took too long: {elapsed} seconds"
...
class TestTimeoutErrorRecovery:
    ...
    async def test_timeout_recovery_in_batch_processing(self):
        ...
        def mixed_success_api(*args, **kwargs):
            ...
            if call_count == 1:
                ...
                return pd.DataFrame({"日期": ["2025-01-01"], "主力净流入-净额": [2000000]})  # Third succeeds
        
        with patch('src.industry_filter.ak.stock_board_industry_name_em', 
                  return_value=mock_industries):
            with patch.object(industry_filter, '_fetch_industry_capital_flow_data_sync',
                             side_effect=mixed_success_api):
                with patch.object(industry_filter, '_fetch_industry_index_data_sync',
                    ...
                    assert elapsed < 3.0, f"Batch processing took too long: {elapsed} seconds"
    ...
    async def test_graceful_degradation_on_widespread_timeouts(self):
        ...
        with patch.object(stock_filter, '_fetch_stock_fund_flow_sync',
                         side_effect=mostly_timeout_api):
            with patch('src.stock_filter.ak.stock_sector_fund_flow_hist',
                ...
                assert elapsed < 5.0, f"Processing with widespread timeouts took too long: {elapsed} seconds"
    ...
    async def test_network_instability_simulation(self):
        ...
        with patch.object(stock_filter, '_fetch_stock_fund_flow_sync',
            ...
            for i in range(5):  # 5 attempts
                tasks.append(
                    stock_filter.process_single_stock_async(f"00000{i+1}", f"股票{i+1}", 29)
                )
            ...
            assert elapsed < 4.0, f"Network instability handling took too long: {elapsed} seconds"
