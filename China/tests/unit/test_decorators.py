...
class TestLoggedDecorator:
    ...
    async def test_logged_async_function_entry_exit(self, caplog):
        ...
        with caplog.at_level(logging.DEBUG, logger="stock_analysis.function_tracker"):
            ...
            # Check log messages
            log_messages = [record.message for record in caplog.records]
            entry_logs = [msg for msg in log_messages if "→ Entering async_simple_function" in msg]
            exit_logs = [msg for msg in log_messages if "← Exiting async_simple_function successfully" in msg]
    ...
    def test_logged_function_with_arguments(self, caplog, sample_test_data):
        ...
        with caplog.at_level(logging.DEBUG, logger="stock_analysis.function_tracker"):
            ...
            log_messages = [record.message for record in caplog.records]
            entry_logs = [msg for msg in log_messages if "→ Entering function_with_args" in msg]
    ...
    def test_logged_function_handles_exceptions(self, caplog):
        ...
        with caplog.at_level(logging.DEBUG):
            ...
            log_messages = [record.message for record in caplog.records]
            error_logs = [msg for msg in log_messages if "✗ Exception in failing_function" in msg]
    ...
    def test_logged_truncates_long_arguments(self, caplog):
        ...
        with caplog.at_level(logging.DEBUG, logger="stock_analysis.function_tracker"):
            ...
            log_messages = [record.message for record in caplog.records]
            entry_logs = [msg for msg in log_messages if "→ Entering function_with_long_args" in msg]
...
class TestTimedAndLoggedDecorator:
    ...
    def test_timed_and_logged_combines_both(self, caplog, capsys):
        ...
        with caplog.at_level(logging.DEBUG, logger="stock_analysis.function_tracker"):
            ...
            log_messages = [record.message for record in caplog.records]
            has_entry = any("→ Entering combined_function" in msg for msg in log_messages)
            has_exit = any("← Exiting combined_function successfully" in msg for msg in log_messages)
    ...
    async def test_timed_and_logged_async_combines_both(self, caplog, capsys):
        ...
        with caplog.at_level(logging.DEBUG, logger="stock_analysis.function_tracker"):
            ...
            log_messages = [record.message for record in caplog.records]
            has_entry = any("→ Entering async_combined_function" in msg for msg in log_messages)
