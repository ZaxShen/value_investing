"""
Simple unit tests for the market_data_fetcher module.
...
async def test_get_stock_market_data_basic(temp_data_dir, mocker):
    ...
    mocker.patch(
        "src.utilities.market_data_fetcher.ak.stock_zh_a_spot_em", return_value=mock_df
    )
...
async def test_get_industry_mapping_data_basic(temp_data_dir, mocker):
    ...
    mocker.patch(
        "src.utilities.market_data_fetcher.ak.stock_board_industry_name_em",
        return_value=mock_industry_names,
    )
    mocker.patch(
        "src.utilities.market_data_fetcher.ak.stock_board_industry_cons_em",
        return_value=mock_industry_stocks,
    )
...
async def test_cache_files_created(temp_data_dir, mocker):
    ...
    cache_files = [
        f for f in os.listdir(temp_data_dir) if f.startswith("stock_zh_a_spot_em_df-")
    ]
